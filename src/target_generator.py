...
valid_classes = [(c, set(c.instances())) for c in ontology.features if len(set(c.instances())) > 0]
...
print(f"Total number of individuals {num_ind}, will need targets with at lead {MIN_IND} to {MAX_IND} individuals")
...
print(f"We have a total of {num_valid_classes} class for targers (features with at least 1 individuals)")
...
while len(simple_targets) < N_SIMPLE_TARGETS:
    ...
    disjuction_terms_ids = np.random.choice(range(num_valid_classes), np.random.randint(2, MAX_DISJUNCTION_TERMS+1), replace=False)
    ...
    print(f"Trying for element {len(simple_targets)} # {counter} - Try {disjuction_terms_ids} of len {len(disjution)}")

    if len(disjution) < MAX_IND and len(disjution) > MIN_IND and disjuction_terms_ids not in simple_targets:
        uniqueness_offset = 0
...
while len(hard_targets) < N_SIMPLE_TARGETS:
    ...
    for _ in range(np.random.randint(2, MAX_DISJUNCTION_TERMS+1)):
        ...
        conjunction_term_ids = np.random.choice(range(num_valid_classes), np.random.randint(2, MAX_CONJUNCTION_TERMS+1), replace=False)
        conjunction_term_ids = tuple(sorted(conjunction_term_ids.tolist()))
        disjunction_terms_ids.add(conjunction_term_ids)

    print(f"Trying for element {len(hard_targets)} # {counter} - Try {disjunction_terms_ids}")
