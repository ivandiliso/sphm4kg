...
class BernoulliMixtureVB(GeneralMixtureModelExponential):
    ...
    def __init__(self, n_components = 2, n_init = 10, n_iter = 100, tol = 1e-3,
                 ...
                 compute_score = False, verbose = False):
        super(BernoulliMixtureVB,self).__init__(n_components,n_iter,tol,alpha0, n_init,
    ...
    def cluster_prototype(self, threshold):
        ...
        prototypes = np.asarray([self.classes_[1*(self.means_[:,i] >= threshold)] for i in
...
class BernoulliMixtureEM(BaseEstimator, ClusterMixin):
    ...
    def _e_step(self, X):
        ...
        for k in range(self.n_components):
            ...
            log_bernoulli = X * np.log(self.theta_[k] + 1e-10) + (1 - X) * np.log(1 - self.theta_[k] + 1e-10)
            log_prob[:, k] = np.sum(log_bernoulli, axis=1) + np.log(self.pi_[k] + 1e-10)

        # Normalize to get responsibilities

        log_prob_norm = log_prob - log_prob.max(axis=1, keepdims=True) # for numerical stability
    ...
    def fit(self, X, y=None):
        ...
        for i in range(self.max_iter):
            ...
            for k in range(self.n_components):
                ...
                log_bernoulli = X * np.log(self.theta_[k]) + (1 - X) * np.log(1 - self.theta_[k])
...
class BernoulliMixtureSGD(BaseEstimator, ClusterMixin):
    def __init__(self, n_components=2, max_iter=1000, tol=1e-4,
                 learning_rate=0.1, batch_size=128, random_state=None,
                 device='cpu'):
        self.n_components = n_components
        self.max_iter = max_iter
        self.tol = tol
    ...
    # Get probabilities from logits
        ...
        log_resp = log_probs_sum.T - torch.logsumexp(log_probs_sum.T, dim=1, keepdim=True)
    ...
    def fit(self, X, y=None):
        ...
        for epoch in range(self.max_iter):
            ...
            loss = -torch.sum(responsibilities * (
    ...
    def _get_param_names(cls):
        return ['n_components', 'max_iter', 'tol', 'learning_rate', 
                'batch_size', 'random_state', 'device']
...
class HierachicalBernoulliMixture(BaseEstimator, ClusterMixin):
    ...
    def fit(self, X, y):
        ...
        self.model_0_ = self.bottom_tier_model(n_components=self.n_components, **self.bottom_tier_args)
    ...
    def _calculate_log_likelihood(self, X, model):
        ...
        log_likelihood_per_component = X @ log_feature_probs + (1 - X) @ log_one_minus_feature_probs
